...
| `45_colour_emoji` | Colour-emoji smoke test for the runa text backend (the default since 1.0). Loads Twemoji-Mozilla (COLRv0) via `font_add_fallback`, renders 🦊 🚀 🐛 🎉 mixed with Latin text in every widget. If you opt back into fontstash with `-define:SKALD_RUNA=false` the emoji render as tofu — there's no equivalent path. |
